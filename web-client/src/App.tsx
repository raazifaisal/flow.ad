...
export default function App() {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (msg.type === 'AUDIO_OUTPUT' && msg.audio) {
          ...
          setMsgs(p => [...p, { id: `ai_${Date.now()}`, sender: 'ai',
  ...
  const selectVibe = (i: number) => {
    setVibeIdx(i);
    const v = VIBES[i];
    const m = { local_event: v.event, environmental_trigger: v.trigger,
  ...
  const sendMsg = () => {
    ...
    setTyping(true);
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'UPDATE_CONTEXT', updateInstruction: txt, manifest }));
    } else {
      setTimeout(() => {
        setTyping(false);
        setMsgs(p => [...p, { id: `ai_${Date.now()}`, sender: 'ai',
